...
module Pf = Printf
module Do = Tiny_httpd_domains

let serve ~config pool new_thread (dir:string) addr port j : _ result =
  let server = S.create ~new_thread ~max_connections:j ~addr ~port () in
...
let main () =
  ...
  let new_thread = Do.new_thread pool in
  match serve ~config pool new_thread !dir_ !addr !port !j with
  | Ok () -> ()
  | Error e ->
    raise e
